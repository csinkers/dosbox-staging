...
bool IsStopped()
{
	return DOSBOX_GetLoop() == DEBUG_Loop;
}

void GetRegisters(LRegisters1& result)
{
...
std::shared_ptr<ILogger> log_(new ConsoleLogger(Debug));
WorkQueue pendingRequests_;
ClientList<ClientConnection> clients_;
ManualResetEvent done_;
std::unique_ptr<std::thread> g_debugThread;
...
#endif // C_DEBUGGER && C_LIZARD_DEBUGGER
