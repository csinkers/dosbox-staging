...
public sealed class ImGuiRenderer : IDisposable // This is largely based on Veldrid.ImGuiRenderer from Veldrid.ImGui
{
    ...
    byte[] LoadEmbeddedShaderCode(ResourceFactory factory, string name)
    {
    ...
    {
        const string prefix = "DosboxDebugger.shaders.";
        var name = prefix + resourceName;
        using Stream? s = _assembly.GetManifestResourceStream(name);

        if (s == null)
        {
            var valid = string.Join(", ", _assembly.GetManifestResourceNames());
